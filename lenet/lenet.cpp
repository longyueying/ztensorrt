...
std::map<std::string, Weights> loadWeights(const std::string file){
    ...
    // Read number of weights blobs
    int32_t count;
    input >> count;
    assert(count > 0 && "Invalid weight map file.");
    std::cout << "There are " << count << " blobs in " << file << std::endl;
    ...
    while (count--){
        ...
        for (uint32_t x = 0; x < size; ++x){
            input >> std::hex >> val[x];
            // std::cout << std::dec << val[x] << std::endl;
        }
        wt.values = val;
        wt.count = size;
        weightMap[name] = wt;
    }
    // std::cout << *((const float*) weightMap["conv1.bias"].values) << std::endl;
    return weightMap;
}
...
ICudaEngine* createLenetEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt){
    ...
    // Add convolution layer which has 6 5*5 filter.
    ...
    // Add max pooling layer with stride of 2*2 and kernel size of 2*2
    IPoolingLayer* pool1 = network->addPoolingNd(*relu1->getOutput(0), PoolingType::kAVERAGE, DimsHW{2, 2});
    assert(pool1);
    pool1->setStrideNd(DimsHW{2, 2});

    //Add second convolution layer which has 16 5*5 filter.
